		...
		<div className='space-y-4'>
			<div className='flex items-center justify-between'>
				<p className='text-sm text-muted-foreground'>
					{isLoading
						? '…'
						: `${stores.length} loja${stores.length !== 1 ? 's' : ''} aguarda${stores.length === 1 ? '' : 'm'} aprovação`}
				</p>
			</div>
						...
						<TableBody>
							{stores.map((store) => {
								const owner = store.users as Record<
									...
									unknown
								>
								const category = store.categories as Record<
									string,
									unknown
								>
								const province = store.provinces as Record<
									string,
									unknown
								>
								return (
