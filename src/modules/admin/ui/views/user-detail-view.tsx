...
export function UserDetailView({ id }: { id: string }) {
	...
	if (!user) return <p className='text-muted-foreground'>Utilizador não encontrado.</p>

	return (
		<div className='max-w-2xl space-y-6'>
			<div className='flex items-center gap-3'>
				<Button render={<Link href='/admin/users'><ArrowLeft className='size-4' /></Link>} variant='ghost' size='sm' />
				<div className='flex items-center gap-3'>
					{user.avatar_url ? (
						<img src={user.avatar_url as string} alt='' className='size-12 rounded-full object-cover border border-border' />
					) : (
					...
					<div>
						<p className='font-heading font-bold'>{`${user.first_name ?? ''} ${user.last_name ?? ''}`.trim()}</p>
			...
			<div className='rounded-2xl border border-border/60 bg-card overflow-hidden'>
				<p className='border-b border-border/60 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground'>Perfil</p>
				<div className='divide-y divide-border/40'>
					<DetailRow label='Nome' value={`${user.first_name ?? ''} ${user.last_name ?? ''}`.trim()} />
					<DetailRow label='Email' value={user.email as string} />
					<DetailRow label='Telefone' value={user.phone_number as string} />
					...
					<DetailRow label='Funções' value={undefined}>
						<div className='flex flex-wrap gap-1'>
							{roles.length > 0 ? roles.map((r) => <StatusBadge key={r} status={r} />) : <StatusBadge status='buyer' />}
						</div>
					</DetailRow>
					<DetailRow label='Criado em' value={user.created_at ? format(new Date(user.created_at as string), 'd MMM yyyy', { locale: pt }) : '—'} />
				</div>
			</div>

			{store && (
				<div className='rounded-2xl border border-border/60 bg-card overflow-hidden'>
					<p className='border-b border-border/60 px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground'>Loja</p>
					<div className='divide-y divide-border/40'>
						<DetailRow label='Nome' value={undefined}>
							<Link href={`/admin/stores/${store.id as string}`} className='text-sm font-medium text-primary hover:underline'>
			...
			<div className='flex flex-wrap gap-2'>
				{hasAdminRole ? (
					<Button
						type='button'
						variant='outline'
						onClick={() => patchMutation.mutate({ removeAdmin: true })}
					...
					<Button
						type='button'
						variant='outline'
						onClick={() => patchMutation.mutate({ makeAdmin: true })}
						disabled={patchMutation.isPending}
					>
						Tornar admin
					</Button>
			...
			<ConfirmDialog
				...
				onConfirm={() => patchMutation.mutate({ status: 'INACTIVE' }, { onSuccess: () => setConfirmAction(null) })}
			/>
			<ConfirmDialog
