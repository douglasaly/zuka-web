...
export async function POST(
	...
	try {
		...
		const storeRating = parseRating(
			(payload as { storeRating?: unknown }).storeRating
		)
		const storeBody = parseBody(
			(payload as { storeBody?: unknown }).storeBody
		)
		...
		for (const item of rawProducts) {
			...
			const productId =
				typeof (item as { productId?: unknown }).productId === 'string'
					...
					: ''
			const rating = parseRating((item as { rating?: unknown }).rating)
			const body = parseBody((item as { body?: unknown }).body)
		...
		if (order.status !== 'COMPLETED') {
			return NextResponse.json(
				{
					error:
						'Só podes avaliar depois de o pedido estar entregue.',
				},
				{ status: 400 }
			)
		}
